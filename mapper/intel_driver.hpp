...
namespace intel_driver
{
	...
	bool CallKernelFunction(HANDLE device_handle, T* out_result, uint64_t kernel_function_address, const A ...arguments) {
		...
		if (ntdll == 0) {
			printf(_("[-] Failed to load ntdll.dll\n"));
			return false;
		}

		const auto NtAddAtom = reinterpret_cast<void*>(GetProcAddress(ntdll, _("NtAddAtom")));
		if (!NtAddAtom)
		{
			printf(_("[-] Failed to get export ntdll.NtAddAtom\n"));
			return false;
		}
		...
		*(uint64_t*)&kernel_injected_jmp[2] = kernel_function_address;
		...
		static uint64_t kernel_NtAddAtom = GetKernelModuleExport(device_handle, intel_driver::ntoskrnlAddr, _("NtAddAtom"));
		if (!kernel_NtAddAtom) {
			printf(_("[-] Failed to get export ntoskrnl.NtAddAtom\n"));
			return false;
		}
		...
		if (original_kernel_function[0] == kernel_injected_jmp[0] &&
			...
			printf(_("[-] FAILED!: The code was already hooked!! another instance of kdmapper running?!\n"));
			return false;
		}
